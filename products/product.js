...
db.product.find({$and: [{product_price: {$not:{$gte:400 ,$lte:800}}}] })
...
db.product
  ...
  .forEach((doc) => {
    db.product.deleteMany({ product_price: doc._id });
  });
...
// db.product.find( { $and: [ { product_color:"indigo" }, { product_price : { $eq : 492 } }]  })
